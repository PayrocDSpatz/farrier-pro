...
// Called during signup or when farrier activates SMS in Settings
// One-time only — if a number is already provisioned for this farrier it will NOT be replaced.
...
// Area codes by US state with city/region labels
// Format: { code, label } — first entry is the default (most populated)
const STATE_AREA_CODES = {
  ...
  IL: [{code:'312',label:'Chicago'},{code:'773',label:'Chicago'},{code:'847',label:'North Suburbs'},{code:'630',label:'West Suburbs'},{code:'708',label:'South Suburbs'},{code:'224',label:'North Suburbs'},{code:'331',label:'West Suburbs'},{code:'815',label:'Rockford/North IL'},{code:'217',label:'Springfield'},{code:'618',label:'Southern IL'}],
  ...
  TX: [{code:'214',label:'Dallas'},{code:'469',label:'Dallas Metro'},{code:'817',label:'Fort Worth'},{code:'713',label:'Houston'},{code:'281',label:'Houston Suburbs'},{code:'832',label:'Houston Metro'},{code:'512',label:'Austin'},{code:'737',label:'Austin Metro'},{code:'210',label:'San Antonio'},{code:'726',label:'San Antonio'},{code:'915',label:'El Paso'},{code:'806',label:'Lubbock/Amarillo'},{code:'903',label:'Northeast TX'},{code:'936',label:'East TX'},{code:'409',label:'Beaumont/SE TX'},{code:'361',label:'Corpus Christi'},{code:'956',label:'Laredo/McAllen'}],
...
function getAreaCodeForState(stateCode) {
  ...
  return codes ? codes[0].code : '561'; // default to 561 if state unknown
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ── IDEMPOTENCY CHECK ──────────────────────────────────────────────────────
    // A Twilio number is a one-time assignment. If this farrier already has one,
    // return it immediately without purchasing a new one.
    try {
      ...
      // Non-fatal — if the check fails, proceed with provisioning anyway
    ...
    // Resolve area code: explicit > state-derived > default 561
