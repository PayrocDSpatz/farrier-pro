...
<body>
  ...
  <script type="text/babel">
    ...
    function FarrierDashboard({ user, userProfile, setUserProfile }) {
      ...
      const tabs = [
        { id: 'qrcode', label: 'My QR Code', icon: '📱' },
      ...
      return (
        <div>
          ...
          <div className="calendar-status">
            ...
            <span>
              Google Calendar: {userProfile?.googleCalendarConnected ? 'Connected ✓' : 'Not Connected'}
            </span>
          ...
          <div className="qr-section">
            <h2 style={{fontSize: '1.75rem', marginBottom: '1rem'}}>
              🚗 Put This QR Code on Your Vehicle
            </h2>
          ...
          <div className="card">
            ...
            <p style={{marginTop: '1rem', color: 'var(--text-muted)'}}>
              💡 Tip: Print multiple copies for your truck, trailer, and business cards. The more visible, the more bookings!
            </p>
          </div>
        </div>
      );
          ...
          <main className="main-content">
            {step === 'success' ? (
              ...
              <div style={{textAlign: 'center'}}>
                <div className="hero">
                  <h1>🐴 {farrierInfo.businessName}</h1>
    ...
    function AppointmentsView({ userId, userProfile }) {
      ...
      const createCalendarEvent = async (appointment, userProfile) => {
        ...
        if (!userProfile?.googleCalendarConnected || !userProfile?.googleAccessToken) {
          console.error('❌ Calendar not connected or no token');
          alert('Please connect Google Calendar first in the "My QR Code" tab');
          return false;
        }

        console.log('✅ Calendar connected, token exists');
        ...
        try {
          ...
          if (appointment.services && appointment.services.length > 0) {
            servicesList = appointment.services.map(s => `   • ${s}`).join('\n');
          } else {
            servicesList = '   (No services specified)';
          }

          const event = {
            summary: `🐴 Farrier - ${appointment.customerName}`,
            description: `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📋 APPOINTMENT DETAILS
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
⏰ Scheduled: ${appointment.requestedDate} at ${appointment.requestedTime}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`,
          ...
          if (db) {
            ...
            console.log('✅ Saved event ID to appointment');
          }
      ...
      const deleteCalendarEvent = async (appointment, userProfile) => {
        ...
        if (!appointment.googleEventId) {
          console.log('⚠️ No Google Event ID found - event may not be in calendar');
        ...
        try {
          console.log('🗑️ Deleting event from Google Calendar:', appointment.googleEventId);
      ...
      const updateStatus = async (appointmentId, newStatus) => {
        console.log('🔵 updateStatus called:', { appointmentId, newStatus });
        ...
        if (db) {
          ...
          if (newStatus === 'confirmed') {
            console.log('🟢 Status is confirmed, attempting calendar sync...');
            ...
            if (appointment) {
              ...
              if (success) {
                alert('✓ Appointment confirmed and added to your Google Calendar!');
              } else {
                alert('⚠️ Appointment confirmed, but failed to add to Google Calendar. Check console for details.');
              }
            } else {
          ...
          if (newStatus === 'cancelled') {
            console.log('🔴 Status is cancelled, removing from calendar...');
            
            if (appointment) {
              const deleted = await deleteCalendarEvent(appointment, userProfile);
              if (deleted) {
                alert('✓ Appointment cancelled and removed from your Google Calendar!');
              } else {
                alert('⚠️ Appointment cancelled in app, but may still be in Google Calendar. You may need to remove it manually.');
              }
            }
          }
                  ...
                  <div key={apt.id} className="appointment-item">
                    <div className="appointment-info">
                      ...
                      <div className="appointment-details">
                        📞 {apt.phone} {apt.email && `• ✉️ ${apt.email}`}<br/>
                        📅 {apt.requestedDate} at {apt.requestedTime}<br/>
                        {apt.services && apt.services.length > 0 && (
                          <>🔧 Services: {apt.services.join(', ')}<br/></>
                        )}
                        {apt.numberOfHorses && (
                          <>🐴 Horses: {apt.numberOfHorses}<br/></>
                        )}
                        {apt.address && (
                          <>📍 {apt.address}, {apt.city}, {apt.state} {apt.zip}<br/></>
                        )}
                        {apt.comments && (
                          <>💬 Notes: {apt.comments}<br/></>
                        )}
    ...
    function InvoicesView({ userId }) {
      ...
      const sendPayrocPaymentLink = async (invoice) => {
        setSendingPaymentLink(invoice.id);
        
        try {
          console.log('📤 Creating Payroc payment link for invoice:', invoice.invoiceNumber);
          
          const response = await fetch('/api/create-payroc-payment', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json'
            },
            body: JSON.stringify({
              ...
              description: `Farrier services - Invoice #${invoice.invoiceNumber}`
            })
          });
          ...
          if (db) {
            await db.collection('invoices').doc(invoice.id).update({
              ...
              paymentLinkSent: firebase.firestore.FieldValue.serverTimestamp()
            });
          }

          alert(`✅ Payment link sent to ${invoice.customerEmail}!\n\nThe customer will receive an email with a secure payment link.`);
          loadInvoices();
          
        } catch (error) {
          console.error('❌ Payment link error:', error);
          alert(`Failed to send payment link: ${error.message}\n\nMake sure Payroc is configured in Vercel environment variables.`);
        } finally {
                ...
                {invoices.map(invoice => (
                  <div key={invoice.id} className="appointment-item">
                    <div className="appointment-info">
                      ...
                      <div className="appointment-details">
                        Amount: ${invoice.total?.toFixed(2)} • Created: {invoice.createdAt?.toDate().toLocaleDateString()}
                        {invoice.paymentUrl && (
                    ...
                    <div className="appointment-actions">
                      ...
                      {invoice.status === 'outstanding' && !invoice.paymentUrl && (
                        <button 
                          ...
                          onClick={() => sendPayrocPaymentLink(invoice)}
                          disabled={sendingPaymentLink === invoice.id}
                        >
                          {sendingPaymentLink === invoice.id ? '⏳ Sending...' : '💳 Send Payment Link'}
                        </button>
                      )}
                      {invoice.status === 'outstanding' && invoice.paymentUrl && (
                        <>
          ...
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            {!paymentUrl ? (
              <button 
                ...
                disabled={loading}
              >
                {loading ? <span className="spinner"></span> : '🔗 Generate Authorize.Net Payment Link'}
              </button>
            ) : (
              <div>
...
# Payroc Payment Links Integration - VERCEL Setup

## Overview
This integration allows you to create and send payment links to customers directly from your Farrier Pro invoices.

## Features
✅ Create payment links for invoices
✅ Automatically email payment link to customer
✅ Support for cards and digital wallets (Apple Pay, Google Pay)
...
### Step 1: Get Payroc Credentials

1. **Sign up for Payroc account:**
   - Go to https://www.payroc.com/
   - Sign up for merchant account
   - Complete merchant application

2. **Get API credentials:**
   - Log into Payroc dashboard
   - Go to Settings → API Keys
   - Copy your **API Key**
   - Copy your **Processing Terminal ID**

### Step 2: Add to Vercel Environment Variables

1. **Go to Vercel Dashboard:** https://vercel.com/dashboard
2. **Select** your farrier-pro project
...
5. **Add these variables:**
   ...
   ```
   Name: PAYROC_API_KEY
   Value: [Your Payroc API Key]
   ☑ Production ☑ Preview ☑ Development
   [Save]
   
   Name: PAYROC_TERMINAL_ID
   Value: [Your Processing Terminal ID]
   ☑ Production ☑ Preview ☑ Development
   [Save]
   
   Name: PAYROC_ENVIRONMENT
   Value: production (or "test" for testing)
   ☑ Production ☑ Preview ☑ Development
   [Save]
   ```
...
```
farrier-pro/
├── index.html (updated with Payroc integration)
└── api/
    └── create-payroc-payment.js (Vercel serverless function)
```

**Steps:**
1. **Create** `api` folder in your GitHub repo
2. **Upload** `create-payroc-payment.js` to the `api` folder
3. **Replace** your `index.html` with the new version
4. **Commit** and push
...
1. **Go to your app** → Invoices tab
2. **Create a test invoice** with your email
3. **Click** "💳 Send Payment Link"
4. **Check your email** for payment link
5. **Click link** → Opens Payroc payment page
6. **Test payment** (use test card if in test mode)
...
1. **Farrier creates invoice** in app
   ```
   Invoice #12345 - John Smith
   Amount: $150.00
   Status: outstanding
   [💳 Send Payment Link]
   ```

2. **Clicks "Send Payment Link"** button
   ```
   ⏳ Sending...
   ```

3. **Backend (Vercel function):**
   - Authenticates with Payroc
   - Creates single-use payment link
...
4. **Customer receives email:**
   ...
   [Pay Now] ← Secure Payroc page
   ```

5. **Customer clicks → Pays:**
   - Opens Payroc hosted payment page
   - Enters card details
   - Completes payment
...
6. **After sending:**
   ...
   Amount: $150.00
   Status: outstanding
   [🔗 View Payment Link] [📱 Send Reminder]
   ```
...
```
POST https://api.payroc.com/v1/processing-terminals/{terminalId}/payment-links
```

### 3. Share Payment Link (Email)
```
POST https://api.payroc.com/v1/payment-links/{paymentLinkId}/sharing-events
```
...
Your app calls:
```
POST /api/create-payroc-payment
...
Body:
{
  ...
  "invoiceNumber": "12345",
  "description": "Farrier services - Invoice #12345"
}

Response:
{
  "success": true,
  "paymentLinkId": "pm_xyz123",
  "paymentUrl": "https://pay.payroc.com/xyz123",
...
**Payroc Fees:**
- Processing: 2.6% + 10¢ per transaction (typical)
- Monthly: Varies by plan
- Setup: Contact Payroc for details

**Example:**
- $150 invoice = ~$4.00 fee
- You receive: $146.00
...
### Test Mode:
1. Set `PAYROC_ENVIRONMENT=test` in Vercel
2. Use test API credentials from Payroc
3. Test endpoint: `https://api.uat.payroc.com`
...
3. Click "Send Payment Link"
4. Receive email
5. Use Payroc test card numbers
6. Verify payment flow

### Get Test Cards:
Contact Payroc support for test card numbers
...
✅ **Single-use payment links** (can't be reused after payment)
✅ **PCI compliance** handled by Payroc
...
**Fix:**
- Check `PAYROC_API_KEY` is correct in Vercel
- Verify API key is active in Payroc dashboard
- Check for extra spaces in environment variable

### Error: "Failed to create payment link"
**Fix:**
- Check `PAYROC_TERMINAL_ID` is correct
- Verify terminal is active in Payroc
...
- Verify Payroc email settings in dashboard
- Check Payroc email sending limits

### Vercel function not working
**Fix:**
- Check Vercel deployment logs
- Verify `api` folder is in root of repo
- Check function name: `create-payroc-payment.js`
- Redeploy after adding environment variables

### Payment link expires
**Note:** Links may expire after certain time
- Check Payroc settings for expiration time
...
- ☑ Terminal ID added to Vercel environment variables
- ☑ Environment set to "production"
- ☑ `api/create-payroc-payment.js` uploaded to GitHub
- ☑ Updated `index.html` uploaded to GitHub
- ☑ Vercel deployment successful
- ☑ Test invoice created and sent
...
To automatically update invoice status when customer pays:

1. **Create webhook endpoint** in `api/payroc-webhook.js`
2. **Register webhook URL** in Payroc dashboard:
   ```
   https://your-site.vercel.app/api/payroc-webhook
   ```
3. **Listen for** `payment.completed` event
4. **Update Firestore** invoice status to "paid"
5. **Send confirmation** to farrier

---

## Support

- **Payroc Support:** support@payroc.com
- **Payroc Documentation:** https://docs.payroc.com
- **Payroc Dashboard:** https://dashboard.payroc.com
...
**Result:** 
Customers click → Pay → You get paid! 💰

No coding required for farriers - it's all built into the app!
